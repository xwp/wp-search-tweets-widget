 ...
 *
 * Based on https://github.com/gedex/WP_Twitter_Client
 */
class Search_Tweets_Widget_Client {
	 ...
	 * Constructor.
	 *
	 * @param Search_Tweets_Widget_Plugin $plugin Array of configuration settings.
	 */
	public function __construct( Search_Tweets_Widget_Plugin $plugin ) {
		$this->plugin = $plugin;

		$this->settings = array(
			'consumer_key'           => $this->plugin->setting->get('consumer_key'),
			'consumer_secret'        => $this->plugin->setting->get('consumer_secret'),

			'oauth_token'            => $this->plugin->setting->get('token'),
			'oauth_token_secret'     => $this->plugin->setting->get('token_secret'),
			'oauth_signature_method' => 'HMAC-SHA1',
			'oauth_timestamp'        => '',
			'oauth_nonce'            => '',
			'oauth_version'          => self::TWITTER_OAUTH_VERSION,

			'user_agent'             => apply_filters( 'search_tweets_widget_client_user_agent', $this->plugin->name . '/' . $this->plugin->version ),
		);
	...
	private function _get_oauth_parameters() {
		...
		return apply_filters( 'search_tweets_widget_client_oauth_parameters', $parameters );
	}
	...
	protected function _get_authorized_header_string() {
		...
		return apply_filters( 'search_tweets_widget_client_authorization_header', 'OAuth ' . implode( ', ', $auth_str ) );
	}
	...
	protected function _get_signature_base_string() {
		...
		return apply_filters( 'search_tweets_widget_client_signature_base_string', $base_string );
	}
	...
	protected function _get_signing_key() {
		return apply_filters( 'search_tweets_widget_client_signing_key', $this->_percent_encode( $this->settings['consumer_secret']) . '&' .  $this->_percent_encode( $this->settings['oauth_token_secret'] ) );
	}
	...
	protected function _get_signature() {
		return apply_filters( 'search_tweets_widget_client_signature', $this->_percent_encode( base64_encode( hash_hmac( 'sha1', $this->_get_signature_base_string(), $this->_get_signing_key(), true ) ) ) );
	}
	...
	public function request( $http_method, $resource, $args = array() ) {
		...
		$this->settings['http_method'] = $http_method = apply_filters( 'search_tweets_widget_client_http_method', strtoupper( $http_method ) );

		$this->settings['base_url'] = apply_filters( 'search_tweets_widget_client_base_url', $base_url );
